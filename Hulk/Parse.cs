 ...
 class Parser
    {
        ...
        private Dictionary<object,Node> variables = new Dictionary<object,Node>();
        ...
        private Stack<Dictionary<object, Node>> functionCallStack = new Stack<Dictionary<object, Node>>();
    ...
    private Node ParseFunctionDeclaration()
{
    ...
    FunctionDeclarationNode functionNode = new FunctionDeclarationNode(functionName, new List<object>(), null);
    ...
    else if (Match(TokenType.Identifier))
{
    ...
    if (Match(TokenType.Parenthesis, "("))
    {
        ...
        {
             FunctionCallNode functionCall = new FunctionCallNode(functionName, arguments,userDefinedFunctions,functionCallStack);
             return functionCall;

            // Verifica si el número de argumentos coincide con la declaración de la función
            if (functionCall.Arguments.Count != arguments.Count)
            {
                throw new Exception($"!SEMANTIC ERROR :La función '{functionName}' recibe '{functionCall.Arguments.Count}' argumentos, no'{arguments.Count} ");
            }
